...
from colorize.common.frame import ImageFrame 
...
if __name__ == '__main__':
    ...
    for project_path in glob.glob(path):
        ...
        line_scene_paths = sorted(glob.glob(os.path.join(project_path, 'sketch/*')))

        for line_scene_path in line_scene_paths:

            color_scene_path = line_scene_path.replace('sketch', 'color')
            if not os.path.exists(color_scene_path):
                continue

            print("Segmenting scene: ", line_scene_path)

            line_paths = glob.glob(line_scene_path + '/*.png')
            color_paths = glob.glob(color_scene_path + '/*.png')

            for line_path, color_path in tqdm(zip(line_paths, color_paths)):
                print(f'Processing line: {line_path}')

                img = cv2.imread(line_path, cv2.IMREAD_UNCHANGED).astype(np.uint8)

                img = ImageFrame._maybe_convert_to_rgba(img)
                ...
                output_path = os.path.join(line_scene_path.replace('sketch', 'tb_seg_neigborhood_51_color_lines_8_5_3'))
