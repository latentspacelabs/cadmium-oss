...
| 4 | Nav-bar Server Settings button — a plain gear icon beside Analyze/Reference (always present, including on hosted backends); click→Settings. Acceleration status (Full speed / Reduced / Optimizing… / Downloading %) lives in the Settings modal's per-capability rows, not the nav bar. | `NavBarIcons.vue`, `NavBar.vue`, `Home.vue`, `ServerSettingsModal.vue` | **shipped** |
| 5 | "Reset embedded backend…" (wipe models/cache/ledger + relaunch as first run), orphan quarantine to `models/.orphaned/` at launch, CoreML-cache wipe on manifest change, sha-verify-on-reuse memoized in the ledger. Still open: gap-bucket `CACHE_KEY` (needs a model re-export at the next models release). | `background.js`, `util/model-hygiene-core.js`, `ServerSettingsModal.vue` | **shipped** |
