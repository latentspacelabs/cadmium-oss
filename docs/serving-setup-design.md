...
| 4 | Nav-bar acceleration chip: Full speed / Reduced (why, click→Settings) / Optimizing… / Downloading %, derived purely in `util/accel-status.js` from the pushed status | `AccelChip.vue`, `NavBar.vue`, `Home.vue` | **shipped** |
